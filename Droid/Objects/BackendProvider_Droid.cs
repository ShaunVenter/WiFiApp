...
{
    public class BackendProvider_Droid
    {
        ...
        public static void SetUserSettings(UserSettings settingsObj)
        {
            ...
            u.DisableConnectionManager = settingsObj.DisableConnectionManager;
        ...
        public static UserSettings GetUserSettings
        {
            get
            {
                try
                {
                    var u = DBHelper.Database.UserSettingsGet();
                    return new UserSettings()
                    {
                        ...
                        DisableConnectionManager = u.DisableConnectionManager,
        ...
        private static async void SetPackagesToServer(List<UserPackage> packages)
        {
            ...
            var updatePackageRanking = await BackendProvider.UpdatePackageRanking(MainApplication.ApiKey, GetUser.UserId, packageIds, packageRanks);
        ...
        public static void UpdatePackagesFromServer()
        {
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    //Read from service
                    var getServicePackages = BackendProvider.GetUserPackagesSync(MainApplication.ApiKey, GetUser.UserId, DBHelper.Database.HotspotHelperGet().SessionID ?? "");
        ...
        public static void GetServiceProviders()
        {
            Task.Factory.StartNew(async () =>
            {
                try
                {
                    ...
                    operation = await BackendProvider.GetServiceProviders(MainApplication.ApiKey);
                        ...
                        using (var client = new HotspotHelperWebClient())
                        {
                            while (SSIDList.Count > 0)
                            {
                                ...
                                try
                                {
                                    ...
                                    var parameters = "?api_key=" + MainApplication.ApiKey + "&ssids=" + WebUtility.UrlEncode(string.Join(",", tempSSIDs));
